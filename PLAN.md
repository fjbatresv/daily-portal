...
- `backend/package.json` — agregar `axios`
- `backend/src/integrations/jira/jira.service.ts`
- `backend/src/integrations/github/github.service.ts`
- `backend/src/telegram/telegram.service.ts`
- Tests unitarios afectados

**Referencias:** `docs/modules/02-github.md` §HTTP client; servicios existentes que usan `fetch`.

**Criterios de aceptación:**

- Todas las llamadas HTTP externas usan `axios` en lugar de `fetch`.
...
- **Tareas completadas:** cuando una task se completa y se abre el PR correspondiente, tachar su número y título en este archivo usando Markdown (ej. `### ~~T08 · JiraModule~~`).
